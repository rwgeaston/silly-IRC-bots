        ...
        if text.find("Place your bets please!") >= 0 and source == 'bowser':
            ...
            if valuations['blue'] - valuations['red'] > 0.5:
                c.privmsg(self.channel, "bowser: %s on blue" %
                          int((valuations['blue'] - valuations['red']) * 30))
            elif valuations['red'] - valuations['blue'] > 0.5:
                c.privmsg(self.channel, "bowser: %s on blue" %
                          int((valuations['red'] - valuations['blue']) * 30))
            else:
                c.privmsg(self.channel, "bowser: %s on a draw" %
                          abs(int((valuations['red'] - valuations['blue']) * 30)))
        elif text.find("luigi just won") >= 0 and source == 'bowser':
            c.privmsg(self.channel, "I'm a weiner!")
